...
{
    internal class DocumentViewAccount : IDocumentView
    {
        protected virtual string ViewName => "home";

        public virtual string SetChildMenu(string html)
        {
            const string selector = "//div[@name='sub-menu-account-{0}']";
        ...
        {
            const string selector = "//*[@id='my-account-parent-option']";
            var doc = ContentSanitizerBase.GetDocument(html);
            if (doc == null) return html;
            var node = doc.DocumentNode;
        ...
        private static readonly List<string> names = [.. "home,profile,permissions".Split(',')];
